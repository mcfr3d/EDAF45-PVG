...
public class LegRaceTest {
	...
	public void test2Etapp() {
		db = new Database(null, Database.LEG_RACE);
		String etappConfig = "{\"etapper\":[{\"minimum time\":\"00.05.00\"},{\"minimum time\":\"00.05.00\"}]}";
		JSONObject root = new JSONObject(new JSONTokener(etappConfig));
		db = new Database(null, Database.LEG_RACE);
		db.setLegInfo(new LegInfo(root.getJSONArray("etapper")));
		tmp = new LegRace(2);
	...
	public void testEtappAllErrors() {
		...
		db = new Database(null, Database.LEG_RACE);
		String etappConfig = "{\"etapper\":[{\"minimum time\":\"00.05.00\"},{\"minimum time\":\"00.05.00\"},{\"minimum time\":\"00.05.00\"},{\"minimum time\":\"00.05.00\"},{\"minimum time\":\"00.05.00\"},{\"minimum time\":\"00.05.00\"}]}";
		JSONObject root = new JSONObject(new JSONTokener(etappConfig));
		db = new Database(null, Database.LEG_RACE);
		db.setLegInfo(new LegInfo(root.getJSONArray("etapper")));
		tmp = new LegRace(6);
	...
	public void testMultiplier() {
		db = new Database(null, Database.LEG_RACE);
		String etappConfig = "{\"etapper\":[{\"minimum time\":\"00.05.00\",\"multiplier\":5}]}";
		JSONObject root = new JSONObject(new JSONTokener(etappConfig));
		db.setLegInfo(new LegInfo(root.getJSONArray("etapper")));
		tmp = new LegRace(1);
		tmp.addTime(new Time("12.00.00", true, 1));
		tmp.addTime(new Time("12.03.00", false, 1));
		String res = "1; 00.15.00; 00.15.00; 12.00.00; 12.03.00";
		assertEquals(res, tmp.genResultWithErrors(null));
	}
